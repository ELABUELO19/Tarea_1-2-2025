...
class MultiModelResult(Base):
    """Modelo para almacenar resultados comparativos de múltiples modelos LLM"""
    __tablename__ = 'multi_model_results'
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    question_id = Column(Integer, nullable=False)  # Referencia a Question.id
    
    # Resultados del modelo GPT-4o-mini
    gpt4o_mini_answer = Column(Text, nullable=True)
    gpt4o_mini_score = Column(Float, nullable=True)
    gpt4o_mini_response_time = Column(Float, nullable=True)
    
    # Resultados del modelo GPT-4o
    gpt4o_answer = Column(Text, nullable=True)
    gpt4o_score = Column(Float, nullable=True)
    gpt4o_response_time = Column(Float, nullable=True)
    
    # Resultados del modelo Mistral-Nemo
    mistral_answer = Column(Text, nullable=True)
    mistral_score = Column(Float, nullable=True)
    mistral_response_time = Column(Float, nullable=True)
    
    # Análisis comparativo
    best_model = Column(String(50), nullable=True)  # Modelo con mejor score
    best_score = Column(Float, nullable=True)       # Mejor score obtenido
    score_variance = Column(Float, nullable=True)   # Varianza entre scores
    
    # Metadatos
    processing_time_total = Column(Float, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    
    # Índices para optimizar consultas
    __table_args__ = (
        Index('idx_multi_model_question_id', 'question_id'),
        Index('idx_multi_model_best_model', 'best_model'),
        Index('idx_multi_model_best_score', 'best_score'),
        Index('idx_multi_model_created_at', 'created_at'),
    )
